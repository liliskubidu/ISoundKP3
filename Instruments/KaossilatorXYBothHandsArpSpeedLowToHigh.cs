...
{
    class KaossilatorXYBothHandsArpSpeedLowToHigh : Instrument
    {
        public string name;
        ...
        public KaossilatorXYBothHandsArpSpeedLowToHigh(string name) : base(name)
        {
            this.name = name;
        ...
        public override bool CheckAndPlayNote(Body body)
        {
            ...
            Result leftHand = GetLeftHand(body);
            Result rightHand = GetRightHand(body);

            float maxX = leftHand.X > rightHand.X ? leftHand.X : rightHand.X;
            float maxY = leftHand.Y > rightHand.Y ? leftHand.Y : rightHand.Y;

            PlaySound(lWristVelocity, maxX, maxY);

            if (maxY > 0.9) maxY = 0.9f;
            if (maxY < 0.5) maxY = 0.5f;
            int speed = (int)(maxY * 127);
            if (true)//body.HandRightState == HandState.Closed)
            {
        ...
        struct Result
        {
            ...
            public Result(float x, float y) : this()
            {
                this.X = x;
                this.Y = y;
            }
        ...
        private Result GetRightHand(Body body)
        {
            //X
            ...
            return new Result(Math.Abs(percentageX), Math.Abs(percentageY));
        }

        private Result GetLeftHand(Body body)
        {
            //X
            ...
            return new Result(1 - Math.Abs(percentageX), Math.Abs(percentageY));
        }
