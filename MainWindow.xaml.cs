namespace GesturalMusic
{
    ...
    {
        string val1 = "void";
        public static bool _arpegiatorSpeed = false;
        public static bool _rightHand = false;
        ...
        public static OutputDevice midiOutput;
        ...
        private void SetRecipient(object sender, RoutedEventArgs e)
        {
            ...
            {
                string selOutput = Convert.ToString(cmbMidiOutput.SelectedValue);
                if (device.Name == selOutput)
                {
                    midiOutput = device;
        ...
        private void SetMode(object sender, RoutedEventArgs e)
        {
            if (AdvancedMode.IsChecked.GetValueOrDefault())
            {
                ...
                if (cbInstruments != null)
                {
                    var selClass = (string)cbInstruments.SelectedValue;
                    Type t = Type.GetType($"GesturalMusic.{selClass}");
                    for (int i = 0; i < instruments.Length; i++)
                    {
                        Instrument instr = (Instrument)Activator.CreateInstance(t, "MIDI");
                        instruments[i] = instr;
                    }
                }
            }
        ...
        public MainWindow()
        {
            ...
            for (int i = 0; i < instruments.Length; i++)
            {
                instruments[i] = new KaossilatorXYRightHand("MIDI");
            }
        ...
        private void InitializeMidi()
        {
            ...
            else
            {
                foreach (OutputDevice device in OutputDevice.InstalledDevices)
                {
                    if (device.Name.Contains("KAOSSILATOR PRO+ 1 SOUND"))
                    {
                        StaticMidi.device = device;
                    }
                }
            }
            if (StaticMidi.device == null) throw new Exception("cannot find MIDIOUT2(USB2.0 - MIDI)");
            StaticMidi.device.Open();
        }
        ...
        private void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            foreach (OutputDevice device in OutputDevice.InstalledDevices)
            {
                cmbMidiOutput.Items.Add(device.Name);
            }

            IEnumerable<Type> instruments = typeof(Instrument)
    .Assembly.GetTypes()
    .Where(t => t.IsSubclassOf(typeof(Instrument)) && !t.IsAbstract);
    //.Select(t => (Instrument)Activator.CreateInstance(t));
            foreach (var item in instruments)
            {
                cbInstruments.Items.Add(item.Name);
            }
        }
        ...
        private void changeValuesinPartitionManager(int number, string instrumentName)
        {
            ...
            instruments[number] = new KaossilatorXYRightHand (instrumentName);
        }
        ...
        private void CbInstruments_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
